...
class NgSshSftp(object):
    def __init__(self, ngwin):
        ...
        for bts in self.bbuip:
            ...
            self.ngwin.logEdit.append('>cd /tmp/node_0xe000/tmp/pm/reports && ls -al && tar -czf /tmp/PM.tar.gz *.xml:')
            stdin, stdout, stderr = ssh.exec_command('cd /tmp/node_0xe000/tmp/pm/reports && ls -al && tar -czf /tmp/PM.tar.gz *.xml')
            self.ngwin.logEdit.append(str(stdout.read(), encoding='utf-8'))
            remotePath = '/tmp/PM.tar.gz'
            localPath = './output/PM_%s.tar.gz' % '_'.join(bts)
            sftp.get(remotePath, localPath)
